...
public class PageNavigator {
	...
	private int currentGroup;		//현재 그룹 (최근 그룹이 0부터 시작) 페이지 1~5까지묶은게 한 그룹
	private int startPageGroup;		//현재 그룹의 첫 페이지
	private int endPageGroup;		//현재 그룹의 마지막 페이지
	private int startRecord;		//전체 레코드-(데이터베이스에 들어있는 행 하나, 게시글 하나) 중 현재 페이지 첫 글의 위치 (0부터 시작)
	
	/*
	 * 생성자
	 */
	public PageNavigator(int countPerPage, int pagePerGroup, int currentPage, int totalRecordsCount) {
		//페이지당 글 수, 그룹당 페이지 수 <앞에 2개는 고정, 현재 페이지(from jsp), 전체 글 수(from DB)<2개는 사용자가 선택하는 것 를 전달받음
